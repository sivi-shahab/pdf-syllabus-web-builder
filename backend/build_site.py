...
def build_site(html_body: str, title: str = "Course Syllabus", out_dir: str = "site") -> Path:
    ...
    if not css_path.exists():
        css_path.write_text(
            """
            body { font-family: system-ui, -apple-system, Segoe UI, sans-serif; margin: 0; background: #f5f5f7; color: #222; }
            header { background: #111827; color: white; padding: 24px; }
            header h1 { margin: 0; font-size: 1.8rem; }
            header p { margin: 6px 0 0; opacity: 0.85; }
            main { max-width: 960px; margin: 24px auto; background: white; padding: 24px 32px; border-radius: 16px;
